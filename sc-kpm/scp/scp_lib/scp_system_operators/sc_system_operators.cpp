    ...
    sc_uint * var_count)
{
  ...
  while (SC_TRUE == sc_iterator3_next(it))
  {
    ...
    if (sc_memory_get_element_type(context, addr3, &element_type) != SC_RESULT_OK)
    {
      continue;
    }
    ...
    sc_type_result * inp_result)
{
  ...
  if (SC_RESULT_OK != sc_memory_get_element_type(context, element, &element_type))
  {
    return SC_FALSE;
  }
  ...
  {
    sc_addr end, begin;
    if (SC_RESULT_OK != sc_memory_get_arc_begin(context, element, &begin))
    {
      return SC_FALSE;
    }
    if (SC_RESULT_OK != sc_memory_get_arc_end(context, element, &end))
    {
      return SC_FALSE;
    }
